...
function createMdcWindow(suspectName)
	...
	guiMdcWindow = guiCreateWindow ( X , Y , Width , Height , "LVMPD - Mobile Data Computer" , false )
	...
	else	-- if it does have a same, call the function getMemoText to get all of the suspects crimes
...
function useMDC(thePlayer, Command)
	...
	isPedInVehicle ( thePlayer))then
		...
		if(isPedInVehicle ( thePlayer)) then
			...
			--check to see if the player is in a police car
			...
			if(vehicle == 596 or vehicle == 427 or vehicle == 490 or vehicle == 599 or vehicle == 601 or vehicle == 523 or vehicle == 597 or vehicle == 598) then
				showLoginWindow()
			else -- player is not in a police car
				outputChatBox("You need to be in a police cruiser.", 255, 0 , 0, true)
			end
			
		else -- player must be next to a PC, show them the MDC
				showLoginWindow()
		end
...
function getMemoText()
	...
	if (suspectDetails[2] == nil ) then
		...
		local intro = "Name: "..suspectDetails[1].."\
		...
		if (suspectCrime) then
			...
			for i, j in pairs(suspectCrime) do
				...
				tempcrime = "\
				--- Crime comitted on "..j[4].. " at "..j[3]..". Crime ID: "..j[1].." --- \
				\
...
function saveSuspectDetails(suspectName, details)
...
addEventHandler("onClientSaveSuspectCrimes", getLocalPlayer() , saveSuspectCrimes)
