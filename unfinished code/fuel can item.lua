...
-- sql --
---------
INSERT INTO items SET id='21', item_name='Fuel Can', item_description='A small red fuel can.';
...
-- use item function
elseif (itemID==21) then -- Fuel Can
...
function getItemsForSale(shop_type, race)
	if(shop_type == 1) then
		-- General Items
		item[21] = {"Fuel Can", "An empty fuel can.", "10", 21, 0, 1, false,}
...
function fillCan(thePlayer, commandName)
	if (isPedInVehicle(thePlayer)) then
		outputChatBox("You need to get out the vehicle to fill a fuel can.", thePlayer, 255, 0, 0)
	else
		local colShape = nil
		...
		if (colShape) then
			...
			-- if there are no fuel can's output a message "you need to buy a fuel can".
