...
function sellVehOffer ( thePlayer, commandName, vehID, targetPlayer , amount )
	...
	if not (vehID) or not (targetPlayer) or not (amount) then -- is the command complete?
		...
		if not (tonumber(owner) == getElementData (thePlayer, "dbid")) -- does the player doing the command own the car?
			outputChatBox("You don't own car ID".. vehID..".", thePlayer, 255, 0, 0)
		else
			...
			if (getDistanceBetweenPoints3D(x, y, z, tx, ty, tz)>4) then -- Are the buyer and seller standing next to each other?
				outputChatBox("You are too far away from this player to sell them a car.", thePlayer, 255, 0, 0) -- error message.
			else
				for i = 1, 128 do -- Storing the offers. Support 128 offers at any one time.
				...
				offerNumber[slot][2] = buyer -- The buyer
				...
				local vehName = getVehicleNameFromModel ( tonumber ( vehModel ) ) -- Getting the vehicle model name for output.
...
function sellVehAccept ( thePlayer, commandName, offerNumber )
	local id = tonumber( offerNumber )
		if not (reports[id]) then -- Is it a valid offer number?
